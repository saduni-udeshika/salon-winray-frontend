...
export default function AllEmpAt() {
    ...
    useEffect(() => {
        const loadPosts =async () =>{
            setLoading(true);
            const response =await axios.get("http://localhost:8070/attendence/");
    ...
    useEffect(() => {
       function getEmpAts() {
           axios.get("http://localhost:8070/attendence/").then((res) =>{
               setEmpAts(res.data);
           }).catch((err) =>{
               alert(err.message);
           })
   ...
   function deleteEmpAt(_id){

        axios.delete("http://localhost:8070/attendence/delete/"+_id).then((res)=>{

            console.log(res.data);

            alert("Employee Attendece Informatin is deleted");
        ...
        setEmpAts(empAts.filter((empAts) => empAts._id !== _id))
    ...
    function createPdf(pdfBody){
        ...
        doc.autoTable({
            didDrawPage: function (data) {
                ...
                // Header
                doc.setFontSize(14);
                var fileTitle = "EMPLOYEE ATTENDENCE REPORT: DATE................";
        ...
        doc.save('attendence.pdf'); //this downloads a copy of the pdf in your local instance.
    };
    ...
    <div className="container">
        <h1>EMPLOYEE ATTENDENCE RECORDS</h1>
        <table id="my-table" class="mye-table">
            <thead>
                ...
                <th className="mye-th" scope="col">Date</th>
            ...
            {loading ? (
                    ...
                    posts
                        .filter((value) => {
                            if (searchName === "") {
                                ...
                                value.date.includes(searchName.toUpperCase())
                            ){
                                return value;
                            }
                    ...
                    <th className="mye-td"  scope="row">COUNT{index+1}</th>
                    <td className="mye-td" >{empAts.empID}</td>
                    <td className="mye-td" >{empAts.empName}</td>
                    <td className="mye-td" >{empAts.date}</td>
                    <td className="mye-td">{empAts.timeIn}</td>
                    <td className="mye-td" >{empAts.timeOut}</td>
                    <td className="mye-td">{empAts.totalHours}</td>
                    
                    <br></br>
                    <div class="btn">
                    <Link to={`/EditempAt/${empAts._id}`} class="btn btn-success btn-sm">Edit
                    &nbsp;
                    <EditIcon className="btn-icon"  fontSize="small"/></Link>
                    &nbsp;
                    
                    <button type="button" class="btn btn-danger btn-sm" onClick={() => deleteEmpAt(empAts._id)}>Delete<DeleteIcon className="btn-icon"  fontSize="small"/></  button>
                    </div>
       ...
       <Link to={"/AddempAt"} className="btn btn-warning btn-sm">ADD NEW RECORD</Link>
       &nbsp;
       &nbsp;
       <button type="button" class="btn btn-danger btn-sm" onClick={() => createPdf()} >Download PDF
