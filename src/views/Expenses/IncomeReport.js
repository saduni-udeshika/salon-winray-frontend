...
function BalanceSheet() {
  ...
  //pdf generation
  function createPdf(pdfBoady) {
    let doc = new jsPDF("portrait", "px", "a3");
    let totalPagesExp = "{total_pages_count_string}"; //placeholder for total number of pages
    doc.autoTable({
      didDrawPage: function (data) {
        // Header
        doc.setFontSize(20);
        let fileTitle = "Balance Sheet";
        var img =
        ...
        doc.addImage(img, "JPEG", 520, 15, 80, 80);
        ...
        doc.autoTable({
          html: "#expensesheet",
          startY: pageHeight - 560,
          theme: "grid",
        });

        doc.autoTable({
          html: "#incomesheet",
          startY: pageHeight - 300,
          theme: "grid",
        });
 ...
 useEffect(() => {
   const loadPosts = async () => {
     setLoading(true);
     const response = await axios.get("http://localhost:8070/expense/");
 ...
 //search income
  ...
  const [postsincome, setPostsincome] = useState([]);
  ...
  return (
    <div className="expenseBody">
      ...
      <div className="containerExpenses">
        ...
        >
            <tr>
              <td className="expenseTableData">Total Expenses: Rs.{sumOfExpense}.00</td>
              <td className="expenseTableData">Total Income: Rs.{sumOfIncome}.00</td>
              <td className="expenseTableData">Profit: Rs.{profit}.00</td>
            </tr>
        </table>
        <div>
        <input
          ...
          >
            ...
            posts
              .filter((value) => {
                if (searchincome === "") {
                  return value;
                } else if (value.date.includes(searchincome)) {
                  return value;
                }
              })
              .map((income) => (
